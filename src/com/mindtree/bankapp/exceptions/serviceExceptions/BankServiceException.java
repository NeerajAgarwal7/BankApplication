package com.mindtree.bankapp.exceptions.serviceExceptions;

import com.mindtree.bankapp.exceptions.daoExceptions.BankDaoException;

public class BankServiceException extends BankDaoException{

	/**
	 * Bank service application exceptions
	 */
	private static final long serialVersionUID = 1L;

	public BankServiceException() {
		super();
	}

	public BankServiceException(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
	}

	public BankServiceException(String message, Throwable cause) {
		super(message, cause);
	}

	public BankServiceException(String message) {
		super(message);
	}

	public BankServiceException(Throwable cause) {
		super(cause);
	}
	
	

}
